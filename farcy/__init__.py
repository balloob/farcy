...
def is_farcy_comment(text):
    """Returns boolean if text was generated by Farcy."""
...
class Farcy(object):
    ...
    def farcy_issue_comments(self, pr):
        """Returns a list of issue comments created by Farcy."""
        return (comment for comment in pr.issue_comments()
                if is_farcy_comment(comment.body))

    def farcy_review_comments(self, pr):
        """Returns a list of review comments created by Farcy."""
        return (comment for comment in pr.review_comments()
                if is_farcy_comment(comment.body))

    def farcy_review_comments_for_file(self, pr, path):
        """For a file, returns a dictionary with Farcy comments per line no."""
        return {comment.position: comment
                for comment in self.farcy_review_comments(pr)
